...
//  Algorithm
//
//  Created by 孙虎林 on 2020/11/27.
//

import Foundation

// 2的倍数
let MapDefaultCapacity = 1 << 3
let MapNotFound = -1

class HashMap {
    typealias Node = KVNode<String, Int>
    typealias K = String
    typealias V = Int
    private var tables: Array<Node>
    private var defaultCapacity = MapDefaultCapacity
    private(set) var size = 0
    
    init(capacity: Swift.Int) {
        defaultCapacity = capacity
        tables = Array<Node>(repeating: Node(k: "", v: -1), count: defaultCapacity)
        tableSetup()
    }
    convenience init () {
        self.init(capacity: MapDefaultCapacity)
    }
    ...
    public func clear() {
        for node in tables {
            if isNullNode(node: node) {
                continue
            } else {
                makeNullNode(node: node)
            }
        }
        size = 0
    }
    public func put(key: K, value: V) -> V {
        let idx = index(key: key)
        let idxNode = tables[idx]
        let newNode = Node(k: key, v: value)
        // 位置是空的.插入
        if isNullNode(node: idxNode) {
            // 根节点
            newNode.parent = nil
            tables[idx] = newNode
        } else {
    ...
    public func remove(key: K) {
        guard let nd = getNode(key: key) else {
            return
        }
        remove(node: nd)
    }
    ...
    public func containKey(_ key: K) -> Bool {
        guard let _ = getNode(key: key) else {
            return false
        }
        return true
    }
    ...
    private func index(key: K) -> Int {
        ...
        // key.hash % index
        return key.hash & (defaultCapacity - 1)
    }
    
    private func isNullNode(node: Node) -> Bool {
        return node.key == "" && node.value == -1
    }
...
//    }
    ...
    private func getNode(key: K) -> Node? {
        ...
        while node != nil {
            if let nd = node {
                let compareValue = compare(key1: key, key2: nd.key)
                if compareValue == 0 {
                    return nd
                } else if compareValue < 0 {
                    node = node?.left
    ...
    private func makeNullNode(node: Node) {
        if isNullNode(node: node) {
            return
        }
        node.key = ""
...
extension HashMap {
    public class func hashMapTest() {
        let map = HashMap()
        _ = map.put(key: "1", value: 1)
        _ = map.put(key: "1", value: 3)
        _ = map.put(key: "2", value: 14)
        _ = map.put(key: "3", value: 13)
        _ = map.put(key: "4", value: 12)
        _ = map.put(key: "5", value: 11)
        print("--")
    }
}
